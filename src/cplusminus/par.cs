...
rule.RuleAction = (pstack) => { pstack.Pop(); pstack.Pop(); pstack.Pop(); pstack.Pop(); return Console.ReadLine(); };
parser1.Rules.Add(rule);
rule = new RGrule("STAT");
rule.RuleAction = (pstack) => { pstack.Pop(); string s = (string)pstack.Pop().Value; pstack.Pop(); pstack.Pop(); Console.WriteLine(": " + s); return ""; };
parser1.Rules.Add(rule);
